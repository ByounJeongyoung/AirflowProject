...
def DataLoad() -> DataFrame:
    ...
    start_date = datetime(2023, month, day,1,minute,second,microsecond)    #임의 지정
    orders_collection = db.orders.find({'createdAt': {"$lte": start_date}},
                                       {'_id': 1,
                                        'realPaidPrice': 1,
                                        'owner': 1,
                                        'payment': 1,
    ...
    dfColumn = df_flatten[['_id', 'createdAt', 'payment_state', 'state', 'realPaidPrice','owner']]
